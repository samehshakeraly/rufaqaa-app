"""ORM mapping for the ``orphan_wishes`` table (migration 0031).

One row per concrete, donor-fundable wish a staff member records for a child.
Mirrors the OrphanSkill org/orphan FK pattern: tenant-owned via
``organization_id`` (every query filters it EXPLICITLY — the app's superuser
connection bypasses RLS) and cascades away with its orphan.

``internal_note`` is STAFF-ONLY free text; the donor composition only ever
projects the WishCard allowlist. ``raised_amount`` is SYSTEM-managed — no R7
...
class OrphanWish(Base):
    __tablename__ = "orphan_wishes"
    __table_args__ = (Index("idx_orphan_wishes_org_orphan", "organization_id", "orphan_id"),)
    ...
    # Coded (WishStatus Literal at the Pydantic boundary; no DB CHECK — the
    # health_status pattern): open / fulfilled / archived.
